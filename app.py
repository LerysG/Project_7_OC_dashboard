...
if rad == '🏠 Home':
    with header:
        ...
        st.markdown("- **Develop a scoring machine learning model** to predict the solvency of clients of a bank-like company (i.e. probability of credit payment failure).")
        st.markdown("It is therefore a **binary classification issue**. Class 0 is solvent client whereas class 1 represents clients with payment difficulties.")
...
if rad == '📈 Test model prediction': 
    with model_predict:
        ...
        if np.isnan(client[info].to_numpy()[0]) == False:
            info_print = round(client[info].to_numpy()[0],1)
        else:
            info_print = "Missing value. Try another one!" 
